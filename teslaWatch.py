...
#### FIXME make this part of the conf input file
TESLA_CLIENT_ID = "????"
TESLA_CLIENT_SECRET = "????"
...
if __name__ == '__main__':
    ...
    try:
        ...
        sys.stderr.write("Error: failed to connect {0}\n".format(e))
        sys.exit(1)
    ...
    #### TODO deal with this when we get the next car
    #### use the VIN to select?
    car = conn.vehicles[0]
 
    # wake up the car and get the current state
    try:
        car.wake_up()
        vState = car.data_request('vehicle_state')
        if options.verbose:
            print "Current state of the car:"
            json.dump(vState, sys.stdout, indent=4, sort_keys=True)
            print ""
 
        cState = car.data_request('climate_state')
        if options.verbose:
            print "Climate state of the car:"
            json.dump(cState, sys.stdout, indent=4, sort_keys=True)
            print ""
 
        dState = car.data_request('drive_state')
        if options.verbose:
            print "Driving state of the car:"
            json.dump(dState, sys.stdout, indent=4, sort_keys=True)
            print ""
    except Exception as e:
        sys.stderr.write("Error: failed to connect to car {0}\n".format(e))
        sys.exit(1)
